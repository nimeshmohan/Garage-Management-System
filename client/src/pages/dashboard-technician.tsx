...
import { PlayCircle, CheckCircle2, History, ClipboardList } from "lucide-react";
...
export function TechnicianDashboard() {
  ...
  const activeJobs = vehicles?.filter(v => v.technicianId === user?.id && v.status !== "Ready for Delivery") || [];
  const completedJobs = vehicles?.filter(v => v.technicianId === user?.id && v.status === "Ready for Delivery") || [];
  ...
  const VehicleCard = ({ v, showActions = true }: { v: any, showActions?: boolean }) => (
    <Card key={v.id} className="border-border/50 shadow-lg relative overflow-hidden group">
      <div className={`absolute top-0 left-0 w-1 h-full ${showActions ? "bg-primary" : "bg-muted-foreground/30"} group-hover:w-2 transition-all duration-300`}/>
